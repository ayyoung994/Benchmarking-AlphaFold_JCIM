...
The no-jump trajectory was centered on `Chain_B` and rewritten using `-center -pbc mol -ur compact`. This step places the protein of interest at the center of the simulation box while reconstructing whole molecules and keeping coordinates compact.
...
### 4. Alignment to the JmjC region of chain B

The centered trajectory was least-squares fitted to `JmjC_ChainB_CA` using `gmx trjconv -fit rot+trans` to provide a stable structural reference frame for downstream ligand-behavior analysis.
